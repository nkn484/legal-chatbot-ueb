...
| AC-004 | REQ-DOC-001 | MUST | Given nguồn nạp, When validate corpus, Then chỉ chấp nhận 3–5 PDF chính thức có text layer, tổng tối đa 200 trang; scan/OCR, DOCX, HTML, TXT bị từ chối; mỗi nguồn có phê duyệt nêu tên trước ingestion. Source registry catalog contract đóng VBQPPL `https://ws.vbpl.vn/vbqppl.asmx` ACTIVE registry/CORE/priority 1 nhưng connector `NOT_IMPLEMENTED`, VNU/UEB PLANNED/LATER/NOT_CONFIGURED không fake endpoint; generic lifecycle vẫn biểu diễn các state catalog. Empty pending allowlist cấm SOAP call; priority nonlegal. Catalog contract validation passed, product behavior vẫn NOT_MEASURED. | NOT_MEASURED |
| AC-005 | REQ-DOC-002 | MUST | Given Curator/Admin, When xử lý một PDF, Then có evidence cho upload, hash, extract, chunk, review thủ công, publish/unpublish và index; Version có SourceDocumentRef lineage chỉ khi `ingestion_origin=SOURCE_FETCH`, còn `UPLOAD` cấm ref. Fetch/extraction không tự publish. | NOT_MEASURED |

---
description: "07.5 - Admin annotation/evaluation"
agent: orchestrator
subtask: false
---

# Thực thi Prompt 07.5 — Admin annotation/evaluation

Prompt ID: `07.5`. Chỉ thực hiện prompt này và phải dừng sau khi submit.

## Gate context tự động

!`python scripts/prompt_gate.py context 07.5`

Nếu `eligible=false`, không sửa file và trả `BLOCKED`. Nếu đủ điều kiện:

1. Đọc `@AGENTS.md` và `@prompts/00_HARD_RULES.md`.
2. Chạy `python scripts/prompt_gate.py check 07.5`.
3. Chạy `python scripts/prompt_gate.py start 07.5`.
4. Kiểm tra `git status --short` và bảo toàn thay đổi hiện hữu.

Dependency trực tiếp: `07.4`.
...
Ngoài đầu ra nêu trên, phải tạo `docs/progress/07.5.md` theo `docs/templates/progress-report.md`. Báo cáo phải ghi lệnh test và kết quả thực tế; không dùng mô tả thay bằng chứng.

Khi hoàn thành, submit bằng một hoặc nhiều `--evidence` là file/test report thực sự:

```bash
python scripts/prompt_gate.py submit 07.5 --report docs/progress/07.5.md --evidence <DUONG_DAN_BANG_CHUNG>
```
